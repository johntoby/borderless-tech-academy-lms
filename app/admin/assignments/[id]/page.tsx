...
export default function AssignmentReviewPage() {
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        ...
        <div className="h-40 bg-[#1E293B] rounded-xl animate-pulse" />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            <span className="text-xs text-slate-500">{assignment.course.title}</span>
      ...
      <Card>
        <h2 className="text-sm font-medium text-slate-400 mb-2">Assignment Description</h2>
        <p className="text-slate-300 text-sm whitespace-pre-wrap">{assignment.description}</p>
        <div className="mt-3 flex gap-4 text-xs text-slate-500">
          <span>Max Score: <span className="text-white font-medium">{assignment.maxScore}</span></span>
          <span>Submissions: <span className="text-white font-medium">{assignment.submissions.length}</span></span>
          <span>Pending: <span className="text-amber-400 font-medium">{pending}</span></span>
          <span>Reviewed: <span className="text-emerald-400 font-medium">{reviewed}</span></span>
        </div>
      </Card>
        ...
        {!assignment.submissions.length ? (
          ...
          <div className="space-y-3">
            {assignment.submissions.map(sub => (
              <Card key={sub.id} className="hover:border-sky-500/30 transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{sub.student.email} · Submitted {formatDateTime(sub.submittedAt)}</p>
                    {sub.submissionText && (
                      <p className="text-sm text-slate-400 mt-2 line-clamp-2">{sub.submissionText}</p>
                    )}
                    {sub.fileUrl && (
                      <a href={sub.fileUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-sky-400 hover:text-sky-300 mt-1">
                        <Download size={11} /> {sub.fileName || 'Download file'}
                      </a>
                    )}
                    {sub.feedback && (
                      <div className="mt-2 text-xs text-slate-400 bg-slate-800 rounded-lg p-2 flex gap-1.5">
        ...
        {gradeModal && (
          <div className="space-y-4">
            <div className="bg-slate-800 rounded-lg p-3">
              <p className="text-sm font-medium text-slate-200">{gradeModal.student.name}</p>
              <p className="text-xs text-slate-400 mt-0.5">Submitted {formatDateTime(gradeModal.submittedAt)}</p>
            </div>
            {gradeModal.submissionText && (
              <div>
                <p className="text-xs font-medium text-slate-400 mb-1">Submission Text</p>
                <div className="bg-slate-800 rounded-lg p-3 text-sm text-slate-300 max-h-40 overflow-y-auto whitespace-pre-wrap">
                  {gradeModal.submissionText}
                </div>
              </div>
            )}
            {gradeModal.fileUrl && (
              <a href={gradeModal.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-sky-400 hover:text-sky-300">
        ...
        {viewSub && (
          <div className="space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                <p className="font-medium text-white">{viewSub.student.name}</p>
                <p className="text-xs text-slate-400">{viewSub.student.email}</p>
              </div>
            ...
            {viewSub.submissionText && (
              <div>
                <p className="text-xs font-medium text-slate-400 mb-1">Submission Text</p>
                <div className="bg-slate-800 rounded-lg p-4 text-sm text-slate-300 whitespace-pre-wrap max-h-60 overflow-y-auto">
                  {viewSub.submissionText}
                </div>
              </div>
            )}
            {viewSub.fileUrl && (
              <a href={viewSub.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-sky-400 hover:text-sky-300">
                <Download size={14} /> {viewSub.fileName || 'Download file'}
              </a>
            )}
            {viewSub.score !== null && (
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
                <p className="text-xs font-medium text-emerald-400 mb-1">Score</p>
                <p className="text-lg font-bold text-white">{viewSub.score} / {assignment.maxScore}</p>
                {viewSub.feedback && <p className="text-sm text-slate-300 mt-2">{viewSub.feedback}</p>}
              </div>
            )}
          </div>
        )}
